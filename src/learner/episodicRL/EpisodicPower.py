...
class EpisodicPower(RLByWeightedML, object):
    ...
    def computeWeighting(self, rewards):
        ...
        weighting=np.exp(self.temperatureScalingPower*(rewards - maxQ) / ((maxQ - minQ) + 10^-6))
        weighting = weighting / np.sum(weighting)

        #FIXME getKLDivergence is commented out on RLByWeightedML where should it be implemented / is it still needed
        self.divKL=self.getKLDivergence(np.onces(np.size(weighting)), weighting)
            
            obj.divKL = obj.getKLDivergence(ones(size(weighting)), weighting);
