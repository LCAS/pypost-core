...
class Sampler(object, SamplerInterface):
    ...
    def finalizeSampler(self, finalizeData):
        ...
        for samplerName in lowLevelsamplers.getKeys():
            ...
            if sampler in self.lowLevelsamplers:
                raise RuntimeError("Added already added lower-level sampler" +
                                   "\"" + samplerName + "\"")
    ...
    def copyPoolsFromSampler(self, sampler):
        ...
        # ASK the line below was commented out in matlab code is there a reason
        # for that? ^mw
    ...
    def isSamplerFunction(self, samplerName):
        if self.getSamplerName() == samplerName:
            ...
            return super(DataManipulatorInterface,
                         self).isSamplerFunction(samplerName)

    def callDataFunction(self, samplerName, newData, *args):
        if self.getSamplerName() == samplerName:
            self._createSamples(newData, args)
        else:
            super(DataManipulatorInterface,
                  self).callDataFunction(samplerName, newData, args)

    # CHANGE we explicitly require a sampler pool in case the pool class will
    ...
    def addSamplerPool(self, samplerPool):
        ...
        for idx, pool in enumerate(self._samplerPoolPriorityList):
            if samplerPool.getPriority() == pool.getPriority():
                raise RuntimeError("A sampler pool with the same priority
                                   already exists")
    ...
    def addLowerLevelSampler(self, samplerPool, lowerLevelSampler,
                             isBeginning):
    ...
    def addSamplerFunctionToPool(self, samplerPool, samplerName, objHandle,
                                 addLocationFlag):
        '''
        # FIXME bad design?
        # FIXME only allow 1
        ...
        The function behaves differently for different values of
        addLocationFlag -1 will add the new samplerfunction to the beginning of
        the sampler pool 0 will flush the entire sampler pool and only add
        samplerPool afterward 1 will add the new sampler function at the end of
        the sampler pool.

        @param samplerPool: sampler pool to whom the sampler function will be
                            included
        ...
        if not self.isSamplerFunction(samplerName):
            raise RuntimeError(samplerName + " is not a valid sampler function
                               of the object")
        ...
        def case0(self):
            self._samplerPools[samplerName].samplerList.insert(0,
                                                               sampleFunction)
    ...
    def createSamplesFromPoolWithPriority(self, lowPriority, highPriority,
                                          data, *args):
        '''
    ...
    # TASK addSamplerFunction referenced in MatLab code does not exists
