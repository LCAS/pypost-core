...
class SequentialSampler(Sampler):
    ...
    - for each step for every episode check bia isActiveSampler witch
    ...
    def addElementsForTransition(self,transitionElementOld,transitionElementNew):
        '''
    ...
    def createSamples(self,data,*args):
        ...
        after that run the appropriate sampler pools for each step and, after checking witch
        episodes are still active, copy the transition elements for the new steo ti the next step
        
        If does that by initializing the dataManager via initSamples() and running createSamplesForStep and
        ...
        while(not finished):
            ...
            if (step>reservedStorage and all(map(lambda x: x.length>0, activeIndexNew))):
    ...
    def selectActiveIdxs(self,data,*args):
        ...
        '''
        isActive=self._isActiveSampler.callDataFunctionOutput('isActiveStep',data,*args)
        tCurrent=args[-1]
    ...
    def _endTransitation(self,data,*args):
        ...
        for old, new in zip(self._transitionElementOldStep, self._transitionElementNewStep):
