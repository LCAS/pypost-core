from src.pypost.optimizer.nloptOptimizers.NLoptOptimizer import NLoptUnconstrained
...
optimizer = NLoptUnconstrained(2, optimizationName=optimizerName, lowerBound=lower_bound)
optimizer.verbose = True
optimizer.expParameterTransform = np.asarray([False, False])

params, value, iterations = optimizer.optimize(rosen, x0=np.asarray([6,6]))
